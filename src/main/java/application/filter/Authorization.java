...
public enum Authorization implements Handler {
    ...
    RequireNotLogin(Filter.of(
            request -> request.jar.get("JSESSIONID").flatMap(request.mustUse(SessionManager.MANAGER)::find).isPresent(),
            Redirection.of(false, "/index.html")
    )),
    // 로그인이 된 경우 강제로 리다이렉션
    RequireLogin(Filter.of(
            request -> request.jar.get("JSESSIONID").flatMap(request.mustUse(SessionManager.MANAGER)::find).isEmpty(),
            Redirection.of(false, "/index.html")
    ));
    // 핸들러 구현부
